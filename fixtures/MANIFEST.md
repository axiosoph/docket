...
| `run-bare-type-pass/` | — (all pass; `docket run czd-oid-disjoint` exits 0) | Not a `C`-check fixture — see below. `src/lib.rs` carries a bare `// @docket: czd-oid-disjoint` (no `:: <command>`) above `pub struct Czd<T>(…)`; the claim is `evaluator: type`, so the marker's mere existence passes it — no command spawned. |
